...
export const studAttendancePeriods = sqliteTable(
	'stud_attendance_periods',
	{
		id: integer('id').primaryKey({ autoIncrement: true }),
		sessionId: integer('session_id')
			.notNull()
			.references(() => studSessions.id, { onDelete: 'cascade' }),
		sectionId: integer('section_id')
			.notNull()
			.references(() => studSections.id, { onDelete: 'cascade' }),
		examTermId: integer('exam_term_id')
			.notNull()
			.references(() => studExamTerms.id, { onDelete: 'cascade' }),
...
export const studAttendanceEntries = sqliteTable(
	'stud_attendance_entries',
	{
		id: integer('id').primaryKey({ autoIncrement: true }),
		sessionEnrollId: integer('session_enroll_id')
			.notNull()
			.references(() => studSessionEnrollments.seid, { onDelete: 'cascade' }),
		periodId: integer('period_id')
			.notNull()
			.references(() => studAttendancePeriods.id, { onDelete: 'cascade' }),
