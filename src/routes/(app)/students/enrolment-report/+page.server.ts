...
export const load: PageServerLoad = async ({ url }) => {
    ...
    function getCasteCategory(casteStr: string | null | undefined): 'General' | 'SC' | 'ST' | 'OBC-A' | 'OBC-B' {
    ...
    for (const cls of classes) {
        ...
        for (const sec of clsSections) {
            const secEnrollments = enrollments.filter(e => e.classId === cls.id && e.sectionId === sec.id);
    ...
    for (const cls of classes) {
        const clsEnrollments = enrollments.filter(e => e.classId === cls.id && e.transferDate === null);
