...
export const load: PageServerLoad = async ({ url }) => {
	...
	const transferredStudents = await db
		.select({
			sid: studInfo.sid,
	...
	if (sidParam) {
		...
		if (!isNaN(sid)) {
			const conditions = [eq(studInfo.sid, sid), isNull(studInfo.transferDate)];
			if (activeSession) {
	...
	if (searchQ && searchQ.trim().length >= 2) {
		const searchConditions = [
			...
			or(like(studInfo.name, `%${searchQ}%`), like(studInfo.portalId, `%${searchQ}%`))
		];
		if (activeSession) {
...
export const actions: Actions = {
	transfer: async ({ request }) => {
		...
		if (activeSession) {
			...
			if (!enrollment) {
				return fail(400, {
					error: 'Cannot transfer a student who is not enrolled in the current active session.'
				});
			}
		}

		await db.update(studInfo).set({ transferDate }).where(eq(studInfo.sid, sid));
	...
	cancelTransfer: async ({ request }) => {
		...
		await db.update(studInfo).set({ transferDate: null }).where(eq(studInfo.sid, sid));
