...
<div class="page-shell" in:fade={{ duration: 400 }}>
	...
	<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
		<!-- Left: Transfer Student Action Panel -->
		<div class="lg:col-span-1 flex flex-col gap-6">
			...
			<div class="action-card">
				...
				<form method="GET" action="" class="search-form flex flex-col sm:flex-row gap-2 mt-4">
					<!-- Maintain selected student in URL query if any -->
					{#if selectedStudent}
				...
				{:else if searchQ && searchQ.trim().length >= 2}
					<p class="no-results-msg mt-4">No active students found matching "{searchQ}".</p>
				{/if}
			</div>
				...
				<div class="action-card highlight-card" in:fade={{ duration: 250 }}>
					...
					<form 
						...
						use:enhance={() => {
							return async ({ update }) => {
								await update();
								// Clear search query and selection after successful transfer
								searchQ = '';
		...
		<!-- Right: Transferred Students List -->
								...
								<tr>
									...
									<td>
										<div class="flex justify-end items-center gap-2">
											<!-- Print Transfer Certificate link -->
											<a 
