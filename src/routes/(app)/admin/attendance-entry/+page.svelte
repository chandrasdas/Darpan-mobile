<script lang="ts">
	import { getPeriodsForSection, getStudentsForAttendance, saveSingleAttendance } from './attendance-entry.remote';
	...
	function handleKeydown(e: KeyboardEvent, r: number, c: number) {
		if (e.key === 'Enter') {
			...
			e.preventDefault();
			const nextInput = document.querySelector(`input[data-row="${r + 1}"][data-col="${c}"]`) as HTMLInputElement;
	...
	let totalTermWorkingDays = $derived(periods.reduce((sum, p) => sum + (p.totalWorkingDays || 0), 0));
	...
	async function fetchSections() {
		if (!currentClass) { sections = []; currentSection = 0; return; }
	...
	async function fetchStudents() {
		if (!currentSession || !currentSection || !currentTerm) { students = []; return; }
	...
	async function handleAttendanceBlur(student: StudentRow, periodId: number, totalWorkingDays: number) {
	...
	<div class="page-hero">
		<div class="hero-content">
			<div class="hero-header">
				...
				<p class="page-subtitle">Enter days present for each student. Changes save automatically when you move to the next field.</p>
			</div>
						...
						{#each periods as p (p.periodId)}
							<th class="text-center w-20">
								<div class="flex flex-col items-center">
									...
									<span class="text-[10px] font-normal text-muted">({p.totalWorkingDays})</span>
								</div>
							</th>
						{/each}
								...
								<td class="relative text-center">
									...
									<div class="absolute right-2 top-1/2 -translate-y-1/2">
										{#if saveStatus[getSaveKey(student.seid, p.periodId)] === 'saved'}
											<span class="status-icon success-icon text-[10px]" in:fade={{ duration: 200 }}>✓</span>
										{:else if saveStatus[getSaveKey(student.seid, p.periodId)] === 'warning'}
											<span class="status-icon warning-icon text-[10px]" title="Exceeds working days">!</span>
...
<style>
	.page-shell { padding: 0 12px 32px; display: flex; flex-direction: column; gap: 16px; }
	@media (min-width: 640px) { .page-shell { padding: 0 20px 32px; gap: 24px; } }
	...
	@media (min-width: 640px) { .hero-content { padding: 24px; border-radius: var(--radius-2xl); } }
	...
	.hero-bottom { display: flex; flex-direction: column; gap: 16px; width: 100%; }
	@media (min-width: 1024px) {
		.hero-bottom { flex-direction: row; align-items: flex-end; justify-content: space-between; }
	}
	.stats-row { display: flex; flex-wrap: wrap; align-items: center; gap: 8px; }
	.stat-item { display: flex; align-items: center; gap: 8px; font-size: 13px; font-weight: 500; color: var(--color-on-surface-variant); }
	...
	.stat-separator { color: var(--color-outline); }
	.hero-filters { width: 100%; }
	@media (min-width: 1024px) { .hero-filters { width: auto; flex-shrink: 0; } }
	.filter-columns { display: flex; flex-direction: column; gap: 16px; align-items: flex-end; }
	.filter-group { display: flex; flex-wrap: wrap; gap: 12px; justify-content: flex-end; width: 100%; }
	.filter-select { min-width: 120px; flex: 1; }
	@media (min-width: 640px) { .filter-group { width: auto; } }
	.filter-divider { height: 1px; width: 100%; background-color: var(--color-outline-variant); }
	.card {
		background-color: var(--color-surface-lowest);
	...
	.data-table {
		...
		text-align: left; font-size: 13px; table-layout: fixed;
	}
	.data-table th {
	...
	.data-table td {
		...
		color: var(--color-on-surface); vertical-align: middle;
		height: 32px; overflow: hidden;
		text-overflow: ellipsis; white-space: nowrap;
	}
	.data-table tbody tr:last-child td { border-bottom: none; }
	.data-table tbody tr { transition: background-color 100ms ease; }
	.data-table tbody tr:hover { background-color: color-mix(in srgb, var(--color-primary) 3%, transparent); }
	.data-table tbody tr:nth-child(even) { background-color: color-mix(in srgb, var(--color-surface-high) 40%, transparent); }
	...
	.success-icon { color: var(--color-status-success-text); font-size: 16px; }
	.warning-icon { color: var(--color-error); font-size: 12px; font-weight: 600; }
	.error-icon { color: var(--color-error); font-size: 16px; }
	...
	.empty-title { font-size: 14px; font-weight: 600; color: var(--color-on-surface); margin: 0 0 4px; }
	.empty-desc { font-size: 14px; color: var(--color-on-surface-variant); margin: 0; }
</style>
